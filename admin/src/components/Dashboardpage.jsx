...
import { Users, UserCheck,CheckCircle, CalendarClock,IndianRupee,XCircle, Search } from "lucide-react"
        ...
        <div className={s.pageContainer}>
            <div className={s.maxWidthContainer}>
                <div className={s.headerContainer}>
                    ...
                    {error && (
                <div className={s.errorContainer}> Error Loading Doctor: {error} </div>
                )}
                </div>
                {/* ststs section*/}
                <div className={s.statsGrid}>
                  ...
                  label="Total Doctors" value={totals.totalDoctors} />
                   <StatCard icon = {<UserCheck className=' w-6 h-6'/>}
                  label="Total Registered user" 
                  value={patientCountLoading ? "Loading..." : (patientCount ?? totals.totalLoginPatients)}
                   />
                   <StatCard icon = {<CalendarClock className=' w-6 h-6'/>}
                  label="Total Appointments" value={totals.totalAppointments}
                   />
                   <StatCard icon = {<IndianRupee className=' w-6 h-6'/>}
                  label="Total Earning" value={`₹ ${totals.totalEarnings}`}
                   />
                ...
                <div className= {s.tableContainer}>
                  <div className={s.tableHeader}>
                     ...
                     <p className={s.tableCount}> {loading ? "Loading..." : `Showing ${visibleDoctors.length} of ${filteredDoctors.length}`}
                       </p>
                  </div>
  ...
  return(
    <div className={s.mobileDoctorCard}>
      ...
      <div className={s.mobileStatsGrid}>
        ...
        <div>
          <div className={s.mobileStatLabel}>Cancelled</div> 
          <div className={s.mobileStatValue + " " + s.textRose500}>{d.appointments.canceled}</div> 
        </div>
        <div>
          <div className={s.mobileEarningsContainer}>Earnings</div> 
