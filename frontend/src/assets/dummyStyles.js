...
export const footerStyles = {
  ...
  desktopEmailInput: "w-full pl-6 pr-36 py-4 text-base text-emerald-800 bg-white border-2 border-emerald-200 rounded-full focus:outline-none focus:ring-4 focus:ring-emerald-300 focus:border-emerald-400 transition-all duration-300 transform shadow-lg placeholder-emerald-400",
...
export const testimonialStyles = {
  ...
  // Scroll container — overflow hidden so only the visible window shows
  scrollContainer: "h-56 sm:h-72 md:h-[360px] lg:h-[400px] overflow-hidden p-3 sm:p-4",
  ...
  // CSS keyframe animations — GPU-composited, no JS scrollTop jumps
  animationStyles: `
    /* Keyframes: upward scroll — translate from 0 to -50% of the duplicated track */
    @keyframes scrollUp {
      0%   { transform: translateY(0); }
      100% { transform: translateY(-50%); }
    }

    /* Keyframes: downward scroll — translate from -50% to 0 */
    @keyframes scrollDown {
      0%   { transform: translateY(-50%); }
      100% { transform: translateY(0); }
    }

    /* Inner track that holds the duplicated cards */
    .scroll-track {
      display: flex;
    ...
    /* Pause on hover or touch — no JS state needed */
    .testimonial-grid:hover .scroll-track,
