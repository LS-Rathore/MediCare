...
// helper function 
// convet time to minutes since midnight
...
// remove duplicate and sort the schedule
...
// convert doctor data to plain text 
...
export async function createDoctor(req, res){
    try {
        ...
        let imageUrl = body.imageUrl || body.imageURL || null;
        ...
        const doc= new Doctor({email: emailLC,
...
export async function deleteDoctor(req, res) {
  try {
    ...
    if(existing.imagePublicId){
        try{
         deleteFromCloudinary(existing.image.publicId);
        }catch(err){
            console.warn("DeleteFromCloudinary error", err?.message || err);
        }
    }
