...
//will get the user from clerk and return the user details 
...
export const getAppointmentsByPatient = async (req, res) =>{
  try {
    ...
    console.log('resolvedCreatedBy (query or clerkUserId): ', resolvedCreatedBy)
  ...
  try {
    ...
    if(existingBooking){
      return res.status(409).json({
        success:false,
        error:"you have already booked an appointment with this doctor for this date and time"
      })
    }
    let doctor = null;
    try{
...
// to confirm the payment and make it paid 
...
export const updateAppointment = async (req,res) =>{
  try {
    ...
    ).populate({path: "docorId", select :"name imageUrl"}).lean();
    return res.json({
      success: true,
      appointment: updated
    })
...
export const getAppointmentByDoctor = async(req,res) =>{
  try {
    ...
    const items = (await Appointment.find(filter)).toSorted({date: 1, time:1}).skip(skip).limit(limit)
    .populate("DoctorId", "name specialization owner imageURL image").lean();
