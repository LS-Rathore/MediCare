  ...
  try {
    ...
    if(!clerkUserId){
      return res.status(401).json({
        success:false,
        message:"Authentication is required to create a service"
      });
    }
...
export const confirmServicePayment = async (req,res) => {
  try {
    ...
    if(!session_id) return res.status(400).json({
        success:false,
        message:"Session id ois required"
    })
    ...
    console.error("Error confirming service payment:", err);
    return res.status(500).json({ success: false, message: "Server error" });
  }
}
...
export const updateServiceAppointment = async (req,res) =>{
    ...
    if(!updated) return res.status(404).json({
      success:false,
      message: "Not found the updated appointment."
    })
    return res.json({
      success:true,
      data:updated
...
export const cancelServiceAppointment = async (req, res) =>{
    ...
    return res.json({
       ...
       data:appt
        ...
        console.error("Error cancelling service appointment:", err);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
...
export const getServiceAppointmentByPatient = async (req, res) => {
    try {
        ...
        console.error("Error in getServiceAppointmentByPatient:", error);
        return res.status(500).json({ success: false, message: "Server error" });
    }
}
